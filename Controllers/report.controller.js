...
const getAllReports = async (req, res) => {
    try {
        ...
        if (reports.length > 0) {
            res.status(200).json({ success: true, data: reports, message: 'Reports retrieved successfully' });
        } else {
            res.status(404).json({ data: [], message: 'No reports found' });
        }
    } catch (error) {
        res.status(500).json({ success: false, data: [], message: 'Error retrieving reports', error: error.message });
    }
};
...
const uploadToSupabase = async (file) => {
    ...
    const { error } = await supabase.storage
        .from('reports') // ✅ GANTI sesuai nama bucket kamu
    ...
    const { data: publicURL } = supabase.storage
        .from('reports') // ✅ GANTI juga di sini
...
const updateReport = async (req, res) => {
    try {
        ...
        if (req.file) {
            newPhotoUrl = await uploadToSupabase(req.file);
            // ❗️Kalau mau hapus file lama dari Supabase, bisa tambahkan di sini
        }
        ...
        report.description = description || report.description;
        report.divisionId = divisionId || report.divisionId;
        if (newPhotoUrl) report.photoUrl = newPhotoUrl;
...
const deleteReport = async (req, res) => {
    try {
        ...
        // ❗️Kamu bisa tambahkan penghapusan file dari Supabase jika perlu
        await report.destroy();
